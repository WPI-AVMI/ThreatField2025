...
def simp_helper(Xs,Ys,ts,Xr,Yr,Tr,U,V,Ks,As):
    q,D, tau = 1,1,0.5
    # Non dimentionalized Concentration Calculations 


    q_on_4piD = q / (4 * np.pi * D)
    r_squared_on_4D = ((Xs - (Xr+U*(ts)))**2 + (Ys-(Yr+V*(ts)))**2)/ (4 * np.pi * D)
    ei2 = expi(-r_squared_on_4D / ts )

    if ts < tau:
        DD = -q_on_4piD * ei2
        return DD
    else:
        DD = q_on_4piD * (expi(-r_squared_on_4D / (ts - tau)) - ei2)
        print(np.amax(DD))
        return DD
...
# === MAIN FUNCTION WITH SINGLE STREAM===
def generate_image_cont_single(Xr, Yr, Tr, U, V, img_height, img_width, Ks, As):
    global current_frame
    sim_size = max([img_height,img_width])
    ...
    for t in range(reps):
        ...
        pixelArray = simp_helper(Xs, Ys, Ts, Xr, Yr, Tr, U, V, Ks, As)
        if scalar is None:
            scalar = np.amax(pixelArray)
            print(scalar)

    
        normalized = (pixelArray / 1 * 255).astype(np.uint8)
        color_mapped = cv2.applyColorMap(normalized, cv2.COLORMAP_JET)
